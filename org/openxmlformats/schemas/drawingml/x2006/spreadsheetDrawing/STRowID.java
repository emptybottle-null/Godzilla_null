...
extends XmlInt {
    ...
    public static final class Factory {
        ...
        public static STRowID newValue(Object object) {
            return (STRowID)type.newValue(object);
        }
        ...
        private static synchronized SchemaTypeLoader getTypeLoader() {
            ...
            if (schemaTypeLoader == null) {
                schemaTypeLoader = XmlBeans.typeLoaderForClassLoader(STRowID.class.getClassLoader());
        ...
        public static STRowID newInstance() {
            return (STRowID)Factory.getTypeLoader().newInstance(type, null);
        }

        public static STRowID newInstance(XmlOptions xmlOptions) {
            return (STRowID)Factory.getTypeLoader().newInstance(type, xmlOptions);
        }

        public static STRowID parse(String string) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(string, type, null);
        }

        public static STRowID parse(String string, XmlOptions xmlOptions) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(string, type, xmlOptions);
        }

        public static STRowID parse(File file) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(file, type, null);
        }

        public static STRowID parse(File file, XmlOptions xmlOptions) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(file, type, xmlOptions);
        }

        public static STRowID parse(URL uRL) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(uRL, type, null);
        }

        public static STRowID parse(URL uRL, XmlOptions xmlOptions) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(uRL, type, xmlOptions);
        }

        public static STRowID parse(InputStream inputStream) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(inputStream, type, null);
        }

        public static STRowID parse(InputStream inputStream, XmlOptions xmlOptions) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(inputStream, type, xmlOptions);
        }

        public static STRowID parse(Reader reader) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(reader, type, null);
        }

        public static STRowID parse(Reader reader, XmlOptions xmlOptions) throws XmlException, IOException {
            return (STRowID)Factory.getTypeLoader().parse(reader, type, xmlOptions);
        }

        public static STRowID parse(XMLStreamReader xMLStreamReader) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(xMLStreamReader, type, null);
        }

        public static STRowID parse(XMLStreamReader xMLStreamReader, XmlOptions xmlOptions) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(xMLStreamReader, type, xmlOptions);
        }

        public static STRowID parse(Node node) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(node, type, null);
        }

        public static STRowID parse(Node node, XmlOptions xmlOptions) throws XmlException {
            return (STRowID)Factory.getTypeLoader().parse(node, type, xmlOptions);
        }

        @Deprecated
        public static STRowID parse(XMLInputStream xMLInputStream) throws XmlException, XMLStreamException {
            return (STRowID)Factory.getTypeLoader().parse(xMLInputStream, type, null);
        }

        @Deprecated
        public static STRowID parse(XMLInputStream xMLInputStream, XmlOptions xmlOptions) throws XmlException, XMLStreamException {
            return (STRowID)Factory.getTypeLoader().parse(xMLInputStream, type, xmlOptions);
        }
