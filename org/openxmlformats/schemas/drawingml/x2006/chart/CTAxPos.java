    ...
    public static final class Factory {
        ...
        private static synchronized SchemaTypeLoader getTypeLoader() {
            ...
            if (schemaTypeLoader == null) {
                schemaTypeLoader = XmlBeans.typeLoaderForClassLoader(CTAxPos.class.getClassLoader());
        ...
        public static CTAxPos newInstance() {
            return (CTAxPos)Factory.getTypeLoader().newInstance(type, null);
        }

        public static CTAxPos newInstance(XmlOptions xmlOptions) {
            return (CTAxPos)Factory.getTypeLoader().newInstance(type, xmlOptions);
        }

        public static CTAxPos parse(String string) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(string, type, null);
        }

        public static CTAxPos parse(String string, XmlOptions xmlOptions) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(string, type, xmlOptions);
        }

        public static CTAxPos parse(File file) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(file, type, null);
        }

        public static CTAxPos parse(File file, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(file, type, xmlOptions);
        }

        public static CTAxPos parse(URL uRL) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(uRL, type, null);
        }

        public static CTAxPos parse(URL uRL, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(uRL, type, xmlOptions);
        }

        public static CTAxPos parse(InputStream inputStream) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(inputStream, type, null);
        }

        public static CTAxPos parse(InputStream inputStream, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(inputStream, type, xmlOptions);
        }

        public static CTAxPos parse(Reader reader) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(reader, type, null);
        }

        public static CTAxPos parse(Reader reader, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTAxPos)Factory.getTypeLoader().parse(reader, type, xmlOptions);
        }

        public static CTAxPos parse(XMLStreamReader xMLStreamReader) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(xMLStreamReader, type, null);
        }

        public static CTAxPos parse(XMLStreamReader xMLStreamReader, XmlOptions xmlOptions) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(xMLStreamReader, type, xmlOptions);
        }

        public static CTAxPos parse(Node node) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(node, type, null);
        }

        public static CTAxPos parse(Node node, XmlOptions xmlOptions) throws XmlException {
            return (CTAxPos)Factory.getTypeLoader().parse(node, type, xmlOptions);
        }

        @Deprecated
        public static CTAxPos parse(XMLInputStream xMLInputStream) throws XmlException, XMLStreamException {
            return (CTAxPos)Factory.getTypeLoader().parse(xMLInputStream, type, null);
        }

        @Deprecated
        public static CTAxPos parse(XMLInputStream xMLInputStream, XmlOptions xmlOptions) throws XmlException, XMLStreamException {
            return (CTAxPos)Factory.getTypeLoader().parse(xMLInputStream, type, xmlOptions);
        }
