    ...
    public static final class Factory {
        ...
        private static synchronized SchemaTypeLoader getTypeLoader() {
            ...
            if (schemaTypeLoader == null) {
                schemaTypeLoader = XmlBeans.typeLoaderForClassLoader(CTTblGrid.class.getClassLoader());
        ...
        public static CTTblGrid newInstance() {
            return (CTTblGrid)Factory.getTypeLoader().newInstance(type, null);
        }

        public static CTTblGrid newInstance(XmlOptions xmlOptions) {
            return (CTTblGrid)Factory.getTypeLoader().newInstance(type, xmlOptions);
        }

        public static CTTblGrid parse(String string) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(string, type, null);
        }

        public static CTTblGrid parse(String string, XmlOptions xmlOptions) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(string, type, xmlOptions);
        }

        public static CTTblGrid parse(File file) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(file, type, null);
        }

        public static CTTblGrid parse(File file, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(file, type, xmlOptions);
        }

        public static CTTblGrid parse(URL uRL) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(uRL, type, null);
        }

        public static CTTblGrid parse(URL uRL, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(uRL, type, xmlOptions);
        }

        public static CTTblGrid parse(InputStream inputStream) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(inputStream, type, null);
        }

        public static CTTblGrid parse(InputStream inputStream, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(inputStream, type, xmlOptions);
        }

        public static CTTblGrid parse(Reader reader) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(reader, type, null);
        }

        public static CTTblGrid parse(Reader reader, XmlOptions xmlOptions) throws XmlException, IOException {
            return (CTTblGrid)Factory.getTypeLoader().parse(reader, type, xmlOptions);
        }

        public static CTTblGrid parse(XMLStreamReader xMLStreamReader) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(xMLStreamReader, type, null);
        }

        public static CTTblGrid parse(XMLStreamReader xMLStreamReader, XmlOptions xmlOptions) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(xMLStreamReader, type, xmlOptions);
        }

        public static CTTblGrid parse(Node node) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(node, type, null);
        }

        public static CTTblGrid parse(Node node, XmlOptions xmlOptions) throws XmlException {
            return (CTTblGrid)Factory.getTypeLoader().parse(node, type, xmlOptions);
        }

        @Deprecated
        public static CTTblGrid parse(XMLInputStream xMLInputStream) throws XmlException, XMLStreamException {
            return (CTTblGrid)Factory.getTypeLoader().parse(xMLInputStream, type, null);
        }

        @Deprecated
        public static CTTblGrid parse(XMLInputStream xMLInputStream, XmlOptions xmlOptions) throws XmlException, XMLStreamException {
            return (CTTblGrid)Factory.getTypeLoader().parse(xMLInputStream, type, xmlOptions);
        }
